...
@php
        ...
        if(isset($crud->entry)) {
            if($crud->entry->id) {
                $alreadySelectedItems = App\Models\Lab\LabGroupItem::where('lab_group_id', $crud->entry->id)->pluck('lab_item_id')->toArray();
            }
        }
...
<script>
    ...
    function lab_general_item(element){
            if($(element).prop('checked') === true){
                ...
                $('#selected_general_item tbody').append('<tr id ="'+element.id+'_general_item"><td class="text-left">'+general_item_name+' (Rs.'+general_item_price+')</td><td><input type="number" value="0" name="display_order['+id+']" class="form-control" /></td></tr>');
