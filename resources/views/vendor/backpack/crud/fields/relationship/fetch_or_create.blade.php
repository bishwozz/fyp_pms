...
if($activeInlineCreate) {
    ...
    if(!isset($inlineCreate)) {
        ...
        if(isset($field['dependencies'])){
            ...
            $field['inline_create']['create_route'] = route($field['inline_create']['entity']."-inline-create-save",[$field['dependencies'][0]=>0]);
            // $field['inline_create']['create_route'] = $field['inline_create']['create_route'];

            //route to modal
            $field['inline_create']['modal_route'] = route($field['inline_create']['entity']."-inline-create",[$field['dependencies'][0]=>0]);
            // $field['inline_create']['modal_route'] = $field['inline_create']['modal_route'];
        }else{
            // dd($field['inline_create']['entity']."-inline-create-save");
            //route to create a new entity
...
if (!window.fetchDefaultEntry) {
    var fetchDefaultEntry = function (element) {
        var $check_dependencies = element.attr('data-dependencies');
...
function setupInlineCreateButtons(element) {
    ...
    $inlineCreateButtonElement.on('click', function () {
        ...
        var loadingText = '<span class="fa fa-spinner fa-spin" style="font-size:18px;"></span>';
...
function triggerModal(element) {
    ...
    $modalSaveButton.on('click', function () {
        ...
        var loadingText = '<i class="fa fa-spinner fa-spin"></i> saving...';
