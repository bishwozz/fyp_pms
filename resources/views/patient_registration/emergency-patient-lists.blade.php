                ...
                <div class="card h-100">
                    <div class="card-header bg-primary p-1">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <a href="{{ backpack_url('/emergency-patient/create') }}" class="btn btn-sm btn-primary float-right createNewPatientBtn" data-toggle="tooltip" title="add"><span class="fa fa-plus font-weight-bold"></span>  {{trans('backpack::crud.add')}}</span></a>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        @foreach ($items as $item)
                                            <tr>
                                                ...
                                                <td><a class="btn-link" href="{{ backpack_url('emergency-patient/'.$item->id.'/edit') }}"  data-toggle="tooltip" title="Edit Patient Details"> {{ $item->patient_no }} </a></td>
                                                <td><a class="btn-link" href="{{ backpack_url('emergency-patient/'.$item->id.'/edit') }}"  data-toggle="tooltip" title="Edit Patient Details"> {{ $item->name }} </a></td>
                                                <td> {{ $item->cell_phone }} </td>
                                                <td>
                                                    <a href="{{ backpack_url('emergency-patient/'.$item->id.'/edit') }}" class="btn btn-sm  btn-success edit-btn mt-2" data-toggle="tooltip" title="Edit Patient Details"><i class="la la-edit"></i></a>&nbsp;
...
<script>
    ...
    $('body').on('keyup', function(e) {
        ...
        if(e.altKey && keyPressed === LMS._formNewFormKey){
           window.location = $('a.createNewPatientBtn').attr('href');
        }
    });
